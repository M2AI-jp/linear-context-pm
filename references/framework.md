...
| **Plan — next atomic change** | Define the next smallest complete checkpoint. Do not create a future-complete backlog outside it, and do not omit required atomic issues inside it. Replacement of stale work removes obsolete material from active `X`; it does not collapse the checkpoint below its required file-changing and Git-state units. A checkpoint is valid only when it has accepted input, process, output, dependencies, downstream consumers, acceptance observation, promotion rule, and state transition contract; a title or phase label is not a checkpoint. |
...
- For repository implementation governed in Linear, file-changing issues and Git-state issues are separate. Create every required atomic issue inside the current checkpoint: one Linear issue changes one file and names that file's single responsibility, inputs, process, outputs, dependencies or interfaces, downstream consumer, and acceptance; "follow SOLID" alone is not enough. A Git-state issue represents only one shared repository transition such as branch/PR creation, review handoff, merge, main promotion, rollback, or reconciliation; do not create Linear issues for local git commands or raw logs. One module PR groups the complete set of file issues and required Git-state work with lint, tests, independent qualitative review of the tests, independent implementation QA, and references between Linear records and Git/PR state.
